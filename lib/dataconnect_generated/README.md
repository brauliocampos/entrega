...
#### Return Type
`execute()` returns a `QueryResult<ListTasksData, void>`
```dart
...
final result = await ExampleConnector.instance.listTasks();
...
```dart
final ref = ExampleConnector.instance.listTasks().ref();
ref.execute();
...
class UpdateTaskVariablesBuilder {
  ...
   UpdateTaskVariablesBuilder isCompleted(bool? t) {
